...
export default function HeroRightCard({ items }: Props) {
   const [activeTab, setActiveTab] = useState<TabId>("shorten");
    ...
    return (
        <Card className='overflow-hidden rounded-xl p-0 shadow-2xl lg:w-10/12 w-full bg-[#F8F9FA] mx-auto h-auto py-3 pb-6'>
            <Tabs defaultValue={items[0]?.topHeading} className="w-full lg:-mt-3 -mt-2 border-none bg-none bg-[#F8F9FA] rounded-none">
                <TabsList className="grid grid-cols-2 w-full rounded-none border-none h-full text-black hover:text-black">
                    {items.map((item, index) => {
                        activeTab === item.id
                        return (
                            <TabsTrigger value={item.topHeading} key={index} onClick={() => setActiveTab("shorten")}  className={cn("w-full flex items-center justify-center cursor-pointer h-10 px-2 bg-[#F8F9FA] hover:opacity-100 shadow-none font-bold  text-black data- active:text-black lg:py-5 py-2 rounded-none border-none", activeTab ? "bg-[#397693] rounded-tl-md rounded-br-none rounded-bl-none rounded-tr-md hover:text-white text-white h-full": "hover:text-black text-black [state=active]:text-black lg:text-lg text-sm px-2")}>
