    ...
    <div className="flex flex-col items-start justify-start gap-y-4 lg:gap-y-6 text-white">
      {items.map((item: HeroLeftProps, index: number) => (
        <div key={index} className="flex flex-col items-start justify-start gap-y-4 w-full lg:gap-y-9">
          <h1 className="text-3xl lg:text-6xl font-bold lg:leading-20 leading-10">{item.heading}</h1>
          <p className="text-md lg:text-xl whitespace-pre-line">{item.description}</p>
          <div className="flex-col lg:flex-row flex items-start justify-start gap-y-4 lg:gap-x-6 w-full mt-4 -lg:mt-10">
            <button className="text-lg lg:text-xl text-center w-full lg:w-auto px-4 lg:px-8 lg:py-6 lg:h-14 py-2 h-10 cursor-pointer flex items-center justify-center text-black bg-white rounded-md">
